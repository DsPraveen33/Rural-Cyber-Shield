...
const SUGGESTED_ACTIONS = [
  { icon: <Link2 className="w-4 h-4" />, label: "Check Link" },
  { icon: <Lock className="w-4 h-4" />, label: "Lock Aadhaar" },
  { icon: <Flag className="w-4 h-4" />, label: "Report Fraud" },
  { icon: <HelpCircle className="w-4 h-4" />, label: "Safety Quiz" },
];
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      role: "ai",
      text: "Namaste! I am Gramin Mitra, your AI cyber safety assistant. How can I help protect you today?",
    }
  ]);
        ...
        onError: () => {
          setMessages(prev => [
            ...prev,
            {
              id: Date.now().toString(),
              role: "ai",
              text: "I'm sorry, I am having trouble connecting right now. Please try again later.",
            }
          ]);
        }
      }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] md:h-[calc(100vh-100px)] animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-primary/5 rounded-xl p-4 border border-primary/10 mb-4 flex items-center gap-3 shrink-0">
        ...
        <div>
          <h2 className="font-bold text-foreground">Gramin Mitra AI</h2>
          <p className="text-xs text-muted-foreground flex items-center gap-1">
            <Shield className="w-3 h-3 text-secondary" /> Secure & Private Chat
          </p>
        </div>
      </div>
